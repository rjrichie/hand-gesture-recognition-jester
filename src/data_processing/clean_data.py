...
test_df = pd.read_csv('dataset/original/annotations/vallist01.txt', sep=' ', header=None, names=['gesture_dir', 'label'])
...
# Remove all classes except for 1, 3, 11, 12, 13, 14, 15, 24, 27
classes_to_keep = [1, 3, 11, 12, 13, 14, 15, 24, 27]
...
reduced_train_df_pre_split = filtered_train_df.groupby('label').sample(n=1030, random_state=random_seed)

reduced_val_df = reduced_train_df_pre_split.groupby('label').sample(n=30, random_state=random_seed)
reduced_train_df = reduced_train_df_pre_split.drop(reduced_val_df.index)
